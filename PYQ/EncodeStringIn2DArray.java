...
public class EncodeStringIn2DArray {
    //It is in O(N^2) Time Complexity which will cause Time limit exceeded exception...So optimize it.
    public static String decodeCiphertext(String encodedText, int rows) {
        ...
        for(int i=0; i<res.length; i++){
            for(int j=0; j<res[0].length; j++){
                res[i][j]=encodedText.charAt(idx++);
            }
        }
        // for(char []c:res){
